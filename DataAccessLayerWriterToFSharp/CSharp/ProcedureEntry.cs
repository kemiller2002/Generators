...
namespace DataAccessLayerWriter
{
    public static class ProcedureEntry
    {
        ...
        public static string ConvertToGetterAndSetter(Field parameter)
        {
            var outputType = (parameter.AllowsNull && !parameter.Type.IsClass) ?
                $"{parameter.Type.Name}?" : parameter.Type.Name;

            return $"public {outputType} {parameter.Name.ToPascalCase()} {{get;set;}}";
        }
                ...
                ProcedureResult result, Dictionary<string, IType> types)
        {
            ...
            var gettersAndSetters = parameters.Select(ConvertToGetterAndSetter)
                    .Join(System.Environment.NewLine);
            ...
            var executeWithExecutionObjectParameters =
                    parameters.Select(x => $"{x.Name.ToCamelCase()} : input.{x.Name.ToPascalCase()}").Join(", ");
...
{{
    ...
    public class {name}ExecutionObject 
    {{
        {gettersAndSetters}    
    }}
    ...
    {{
        ...
        public {executionResult} ExecuteWithExecutionObject({name}ExecutionObject input)
        {{
            
            return Execute({executeWithExecutionObjectParameters});
